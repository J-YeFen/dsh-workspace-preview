...
const CHUNK_NAMES = ['office', 'editor'];
// (editor = CodeMirror chunk, built by scripts/build-editor.mjs from devDeps)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const info = await fs.stat(target);
        if (!info.isFile()) throw new Error('not a file');
        if (info.size > MAX_READ_BYTES) throw new Error('html file exceeds the 3 MiB cap');
        const body = await fs.readFile(target);
        writeRaw(res, 200, {
