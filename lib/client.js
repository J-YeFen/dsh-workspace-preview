window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 直接渲染为页面的扩展名（沙箱 iframe，见 HtmlView）
    ...
    const CSS = [
      ...
      // 本地图片占位（假 https 域名）在换成 data URL 前隐藏，避免破图闪烁
      ".fx-md img[src^=\"https://dsh-workspace-preview.invalid/\"]{visibility:hidden}",
      ...
      ".fx-html-wrap{flex:1;overflow:auto;background:var(--dsw-alias-bg-base)}",
      ...
      // 编辑器（等宽 textarea 铺满预览体）
      ".fx-editor{flex:1;min-height:0;width:100%;resize:none;border:none;outline:none;padding:12px 14px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.6}",
      ...
      // Office 预览：docx 文档 / xlsx 表格 / pptx 幻灯片卡片
    ...
    // ── Markdown 预处理（关键：宿主 MarkdownText 的安全策略）──────────────
    // 宿主渲染器把原始 HTML 当字面文本显示（不进入 DOM），且图片只放行绝对
    // http(s) URL（`data:` URI 只渲染出 alt 文本）。若把本地图片直接内联成
    // `data:image/png;base64,...` 塞进原文，README 开头的 `<p align="center">
    // <img src="data:...">` 整段 HTML 会以字面文本形式显示——一个 ~1MB 的
    // base64 墙，正是此前预览 README 出现的"乱码"。因此这里在交给渲染器之前：
    //   1. 剥掉所有原始 HTML 标签（只留文本内容，`<br>` 换行）；
    //   2. 本地图片引用（`![alt](路径)` 与 `<img src>`）改写成指向保留域名的
    //      假 https URL `https://dsh-workspace-preview.invalid/{id}`——绝对
    //      https 通过宿主放行，渲染成真正的 `<img>` 元素；
    //   3. 渲染完成后（MdView 内）再把这些假 URL 的 src 换成 RPC 读出的
    //      data URL。外部 http(s) 图片保留原样，由宿主直接渲染。
    const MD_IMG_CACHE = new Map();
    const MD_IMG_CACHE_MAX = 64;                     // 缓存条数（FIFO 淘汰）
    const MD_IMG_LIMIT = 32;                         // 单篇文档最多内联图片数
    const MD_IMG_MAX_BYTES = 1024 * 1024;            // 单图解码字节上限
    const MD_IMG_TOTAL_BUDGET = 8 * 1024 * 1024;     // 整篇 data URL 字符串预算

    // .invalid 是保留 TLD（RFC 2606），永远不会解析成功；仅用作渲染占位
    const MD_IMG_FAKE_HOST = "https://dsh-workspace-preview.invalid/";
    ...
    function prepareMarkdownText(text, dir) {
      const jobs = [];                             // {id, path, alt}：本地图片抓取任务
      ...
      for (const line of lines) {
        ...
        out.push(transformLine(line, dir, jobs));
      }
      return { text: out.join("\n"), jobs };
    }

    // 单行变换：行内代码遮罩 → `<img>` / `![...]` 改写 → 剥 HTML 标签 → 还原遮罩
    function transformLine(line, dir, jobs) {
      ...
      // `<img ...>`：本地路径 → 假 https URL 占位；外部 http(s) → markdown 图片语法
      s = s.replace(/<img\b([^>]*)>/gi, (m, attrs) => {
        ...
        if (p && IMG_EXT.has(extOf(p)) && jobs.length < MD_IMG_LIMIT) {
          const id = jobs.length; jobs.push({ id, path: p, alt });
          return `![${alt}](${MD_IMG_FAKE_HOST}${id})`;
        }
        if (/^https?:/i.test(src)) return alt ? `![${alt}](${src})` : `![](${src})`;
        return alt || m;                            // data:/其它：保留 alt 文本或原标签（随后被剥掉）
      });
      // `![alt](src)`：本地路径 → 假 https URL 占位；其余（外部 http(s)、data: 等）原样保留
    ...
    // ── Office 预览（docx / xlsx / pptx，数据由 host 端 lib/office.js 提取）──
    ...
    // ── 预览视图组件（React.memo：大文件/重渲染不随目录树交互反复重算）──

    // HTML：沙箱 iframe 渲染为页面（sandbox="" 禁用脚本，工作区内的 HTML
    // 永远不会在宿主页面上下文执行任意代码）。
    function HtmlView({ text, truncated }) {
      return el("div", { className: "fx-preview-body" },
        el("div", { className: "fx-html-wrap fx-scroll" },
          el("iframe", {
    ...
    // Markdown：GFM 渲染 + 内嵌图片解析（见 prepareMarkdownText / transformLine）。
    // 关键点：宿主 MarkdownText 把原始 HTML 当字面文本、只放行绝对 http(s) 图片，
    // 因此本地图片先改写为 `https://dsh-workspace-preview.invalid/{id}` 占位（绝对
    // https 会被渲染成真正的 <img>），渲染完成后用 RPC 读出的 data URL 换掉 src。
    // 预处理在渲染前同步完成，避免先闪一帧原始 HTML 字面文本；layout effect 在
    // 浏览器绘制前换 src，配合 CSS 隐藏占位，不会出现破图闪烁。
    const MD_IMG_BLANK = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
    function MdView({ rpc, text, dir, truncated }) {
      const rootRef = useRef(null);
      const prepared = useMemo(() => prepareMarkdownText(text, dir), [text, dir]);
      const [imgState, setImgState] = useState(null);  // {text, urls}；null = 未就绪

      useEffect(() => {
        let alive = true;
        const jobs = prepared.jobs;
        setImgState(null);                            // 文本变化时先清空旧映射，防止错配
        if (jobs.length === 0) { setImgState({ text: prepared.text, urls: new Map() }); return undefined; }
        const urls = new Map();
        let totalBytes = 0;
        (async () => {
          await runPool(jobs, async (job) => {
            let url = MD_IMG_CACHE.get(job.path);
            if (!url) {
              try {
                const res = await rpc.call("/dsh-workspace-preview", "read", { path: job.path });
                const v = res?.ok && res.value;
                if (v && v.kind === "image" && (v.size ?? 0) <= MD_IMG_MAX_BYTES) {
                  url = `data:${v.mime};base64,${v.base64}`;
                  MD_IMG_CACHE.set(job.path, url);
                  if (MD_IMG_CACHE.size > MD_IMG_CACHE_MAX) {
                    MD_IMG_CACHE.delete(MD_IMG_CACHE.keys().next().value);
                  }
                }
              } catch { /* 保留占位 */ }
            }
            if (url && totalBytes + url.length <= MD_IMG_TOTAL_BUDGET) {
              totalBytes += url.length;
              urls.set(job.id, url);
            }
          }, 6);
          if (!alive) return;
          setImgState({ text: prepared.text, urls });
        })();
        return () => { alive = false; };
      }, [rpc, prepared]);

      // 换 src：占位 https → data URL；读取失败/超预算的换成透明像素（保留 alt）。
      // 只改属性不增删节点，React 再次渲染时也能自愈。
      useLayoutEffect(() => {
        const root = rootRef.current;
        if (!root || !imgState || imgState.text !== prepared.text) return;
        const imgs = root.querySelectorAll(`img[src^="${MD_IMG_FAKE_HOST}"]`);
        for (const img of imgs) {
          const id = Number(img.getAttribute("src").slice(MD_IMG_FAKE_HOST.length));
          img.setAttribute("src", imgState.urls.get(id) ?? MD_IMG_BLANK);
        }
      }, [prepared.text, imgState]);

      return el("div", { className: "fx-preview-body" },
        el("div", { className: "fx-md-scroll fx-scroll" },
          el("div", { className: "fx-md", ref: rootRef },
            el(MarkdownText, { text: prepared.text }))),
        truncated
          ? el("div", { className: "fx-trunc-note" },
              "内容过长，仅渲染前 256 KiB，完整内容请用外部程序打开")
          : null,
    ...
    function FileExplorer(props) {
      ...
      const canEdit = preview.phase === "ready" && preview.data.kind === "text" && !preview.data.truncated;
      const dirty = editing && preview.phase === "ready" && draft !== preview.data.text;
      ...
      const saveEdit = async () => {
        ...
        try {
          ...
          setPreview({ phase: "ready", data: { ...preview.data, text: draft, truncated: false, size: res.value.bytes, mtimeMs: res.value.mtimeMs } });
      ...
      if (!selected) {
      ...
      } else if (editing && preview.data.kind === "text") {
        // 编辑态：等宽 textarea 铺满预览体，Cmd/Ctrl+S 保存
        previewBody = el("textarea", {
          className: "fx-editor fx-scroll",
          value: draft,
          spellCheck: false,
        ...
        if (data.kind === "text") {
          ...
          if (MD_EXTS.has(ext)) {
            // Markdown：GFM 渲染 + 内嵌图片解析（相对/绝对路径经 RPC 读成 data URL）
            ...
            // HTML：沙箱 iframe 渲染为页面（sandbox="" 禁用脚本，工作区内的
            // HTML 永远不会在宿主页面上下文执行任意代码）。
            previewBody = el(MemoHtmlView, { key: selected.path, text: data.text, truncated: data.truncated });
          } else {
            // 代码/文本：复用 dsh 的 read 卡片 —— 行号 + shiki 语法高亮 + 复制按钮。
            previewBody = el(MemoCodeView, { key: selected.path, text: data.text, ext, truncated: data.truncated });
          }
          ...
          // Office（host 端已解析成结构化数据）；key 保证换文件时重置内部状态
          previewBody = data.format === "docx" ? el(DocxView, { key: selected.path, data })
            : data.format === "xlsx" ? el(XlsxView, { key: selected.path, data })
            : el(PptxView, { key: selected.path, data });
        } else {
