window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // PDF 由浏览器内建引擎渲染（iframe 直连 /media 路由的原始字节）
    ...
    // docx/pptx 富预览：拉整份原始字节给浏览器端渲染器（office-rich 懒 chunk）
    const officeRawUrlOf = (p) => `${origin()}/dsh-workspace-preview/office-raw?path=${encodeURIComponent(normalizeAbsPath(p))}`;
    ...
    //   'pdf'   —— 不取数，iframe 直连 /media 路由（浏览器内建 PDF 引擎，v0.3.4）
    ...
    const viewerDefs = [
      ...
      { id: "pdf", exts: PDF_EXTS, needs: "pdf", priority: 45 },
    ...
    const CSS = [
      ...
      // PDF 预览：iframe 直连 /media 路由（浏览器内建 PDF 引擎；不带 sandbox 属性，
      // Chromium/Edge 的 PDF 引擎拒绝在沙箱 iframe 里工作——见 PdfView 注释）
      ".fx-pdf-frame{display:block;width:100%;height:100%;border:0;background:var(--dsw-alias-bg-base)}",
    ...
    // PDF：<iframe> 直连 /media 路由的原始字节，交给浏览器内建 PDF 引擎渲染
    // （Chromium/Edge 的 PDFium、Firefox 的 PDF.js）。
    // 这里刻意不带 sandbox="" 属性：Chromium/Edge 的 PDF 引擎在沙箱 iframe 里
    // 会被直接拒绝渲染（hedgedoc、wordpress-playground 等内嵌场景踩过同款坑）。
    // 边界仍成立：/media 路由本身过浏览器信任围栏（loopback / trustedHosts），
    // PDF 不是 HTML——由引擎在浏览器自己的 PDF 沙箱里解析，不会以本页 origin 执行。
    ...
    // docx/pptx 富预览：懒 chunk（/bundle/office-rich.js，esbuild 一次性打包
    // docx-preview + jszip，产物已提交）。组件从 /office-raw 拉整份原始字节
    // 自己渲染（docx-preview / pptx 画布），失败时给出精确审阅(本机 Office)入口。
    function LazyOfficeRichView({ url, ext, onOpenExternal }) {
      const [state, setState] = useState({ status: "loading" });
      useEffect(() => {
        let alive = true;
        loadChunk("office-rich").then((mod) => {
          if (!alive) return;
          const View = ext === "pptx" ? mod?.PptxRichView : mod?.DocxRichView;
          if (typeof View !== "function") {
            setState({ status: "error", message: "office-rich chunk 缺少渲染组件" });
            return;
          }
          setState({ status: "ready", Comp: View });
        }).catch((error) => {
          if (!alive) return;
          setState({ status: "error", message: String(error?.message ?? error) });
        });
      ...
      if (state.status === "loading") {
        return el("div", { className: "fx-empty" },
          el("span", { className: "fx-spin" }), el("span", null, "加载富预览…"));
      }
      if (state.status === "error") {
        return el("div", { className: "fx-notice" },
          el(CloseIcon, { width: 26, height: 26, className: "fx-notice-ico" }),
          el("span", null, "富预览加载失败"),
          el("span", { className: "fx-dim" }, state.message),
          onOpenExternal
            ? el("button", { type: "button", className: "fx-mini-btn", onClick: () => onOpenExternal() },
                "改用本机 Office 打开")
            : null);
      }
      return el(state.Comp, { url, onOpenExternal });
    }
    ...
    const MemoPdfView = React.memo(PdfView);
    const MemoOfficeView = React.memo(LazyOfficeView);
    const MemoOfficeRichView = React.memo(LazyOfficeRichView);

    // ── the FileExplorer surface ────────────────────────────────────────────

    // 面板几何宽度由 dsh-sidebar-shell 统一持有（sidebarWidth / detailWidth），
    // 这里只保留拖拽时的 clamp 边界（与 shell 的 SIDEBAR_*/DETAIL_* 一致）。
    // PREVIEW_MAX 放宽到 4096:真正的“接近左边栏留缝隙”由 shell 在 setDetailWidth
    // 里按视口 clamp(viewport - DOCK_GAP),面板不再自行限死 960。
    const TREE_MIN = 200, TREE_MAX = 480;
    const PREVIEW_MIN = 320, PREVIEW_MAX = 4096;
    const MEDIA_CAP = 20 * 1024 * 1024; // 与 host 媒体路由一致；超过则给提示而非破图

    function FileExplorer(props) {
      // shell 注入的共享上下文 + 面板几何（宽度由 dsh-sidebar-shell 统一持有）
      const { rpc, openPath, sidebarWidth, setSidebarWidth, detailWidth, setDetailWidth, reportLayout } = props;
      ...
      // 树列始终显示（收起/展开由 shell 的 tab 与最小化按钮负责）。
      ...
      const openFile = (entry) => {
        ...
        if (viewer.needs === "pdf") {
          // PDF：不取正文（绕开 3 MiB read 上限），iframe 直连 /media 路由
          // 原始字节交给浏览器内建 PDF 引擎；超过媒体路由 20 MiB 才给提示
          if (entry.size != null && entry.size > MEDIA_CAP) {
            setPreview({ phase: "ready", kind: "too-large-media", size: entry.size });
          } else {
            setPreview({ phase: "ready", kind: "pdf", url: mediaUrlOf(entry.path), name: entry.name });
          }
          return;
        }
        if (viewer.needs === "data" && (ext === "docx" || ext === "pptx")) {
          // docx/pptx 富预览(v0.3.5):不再走 RPC read 的文本提取,直接交给
          // office-rich 懒 chunk——它从 /office-raw 拉整份原始字节自渲染
          // (docx-preview / pptx 画布),超过 30 MiB 等失败会在组件内提示。
          setPreview({ phase: "ready", kind: "office-rich", url: officeRawUrlOf(entry.path), ext });
          return;
        }
        (async () => {
          try {
      ...
      // 面板总宽度上报给 shell（shell 据此对 app 框架做 padding-right 位移）。
      const rootRef = useRef(null);
      const previewOpen = selected !== null;
      const dockWidth = sidebarWidth + (previewOpen ? detailWidth : 0);
      useLayoutEffect(() => {
        reportLayout(dockWidth);
      }, [dockWidth, reportLayout]);
      ...
      const treeResize = resizeHandlers(() => sidebarWidth, setSidebarWidth, TREE_MIN, TREE_MAX);
      const previewResize = resizeHandlers(() => detailWidth, setDetailWidth, PREVIEW_MIN, PREVIEW_MAX);
      ...
      if (!selected) {
        ...
        if (kind === "html") {
          ...
          // PDF：iframe 直连 /media 路由（浏览器内建 PDF 引擎；无 sandbox 属性）
          previewBody = el(MemoPdfView, { key: selected.path, url: preview.url });
        ...
        } else if (kind === "office-rich") {
          // docx/pptx 富预览：office-rich 懒 chunk 拉 /office-raw 自渲染；
          // onOpenExternal = 精确审阅(openPath 调本机 Office)
          previewBody = el(MemoOfficeRichView, {
            key: selected.path,
            url: preview.url,
            ext: preview.ext,
            onOpenExternal: () => void openPath(selected.path),
          });
        } else {
      ...
      return el("div", { ref: rootRef, className: "fx-root" + (dragging ? " fx-dragging" : "") },
        previewOpen && el("section", {
          ...
          style: { right: sidebarWidth, width: detailWidth },
          "aria-label": "文件预览",
        },
    ...
    // 面板注册进共享外壳（dsh-sidebar-shell）。外壳未就绪时先入队，外壳加载后
    // 自行 drain。面板不再自挂 shell.overlay / 改 padding-right / 画悬浮条。
    const registerPanel = (desc) => {
      const api = window.__dshSidebarShell__;
      if (api) return api.register(desc);
      (window.__dshSidebarShellQueue__ = window.__dshSidebarShellQueue__ || []).push(desc);
      return () => {};
    };

    // 面板不再需要任何 client 服务：rpc / openPath 由 shell 统一注入。
    const inject = [];
