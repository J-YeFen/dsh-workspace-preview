window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── Markdown 内嵌图片解析 ──────────────────────────────────────────────
    // MarkdownText（宿主渲染器）无法注入自定义 img 处理器，因此把 `![alt](src)`
    // 与 `<img src>` 里的本地引用解析为工作区绝对路径，经 RPC read 读成 data URL
    // 后再交给渲染器。会话级缓存避免反复抓取；数量/单图/总预算三重上限防大文档卡顿。
    ...
    // 收集 `![alt](src)` 与 `<img src>` 两类图片引用
    function collectImageRefs(text) {
      const refs = [];
      const mdRe = /!\[([^\]]*)\]\(([^()\s]+)(?:\s+["'][^"']*["'])?\)/g;
      let m;
      while ((m = mdRe.exec(text))) refs.push({ raw: m[0], alt: m[1], src: m[2] });
      const imgRe = /<img\b[^>]*\bsrc=(["'])(.*?)\1[^>]*>/gi;
      while ((m = imgRe.exec(text))) refs.push({ raw: m[0], src: m[2] });
      return refs;
    }
    ...
    // Markdown：GFM 渲染 + 内嵌图片解析（见 collectImageRefs / resolveWorkspacePath）。
    // 图片一次性并发抓取并写入 data URL，期间先渲染原文，解析完成再整体替换。
    function MdView({ rpc, text, dir, truncated }) {
      ...
      useEffect(() => {
        ...
        // 去重并解析为本地路径（按数量上限裁剪，超出的保留原始引用）
        const byPath = new Map();
        for (const r of refs) {
          const p = resolveWorkspacePath(dir, r.src);
          if (!p || !IMG_EXT.has(extOf(p)) || byPath.size >= MD_IMG_LIMIT) continue;
          let job = byPath.get(p);
          if (!job) { job = { refs: [] }; byPath.set(p, job); }
          job.refs.push(r);
        }
        ...
        let totalBytes = 0;
        (async () => {
          await runPool([...byPath.keys()], async (p) => {
            let url = MD_IMG_CACHE.get(p);
            if (!url) {
              try {
                const res = await rpc.call("/dsh-workspace-preview", "read", { path: p });
                const v = res?.ok && res.value;
                if (v && v.kind === "image" && (v.size ?? 0) <= MD_IMG_MAX_BYTES) {
                  url = `data:${v.mime};base64,${v.base64}`;
                  MD_IMG_CACHE.set(p, url);
          ...
          if (!alive) return;
          let out = text;
          for (const [p, job] of byPath) {
            const url = urls.get(p);
            if (!url) continue;
            for (const r of job.refs) {
              out = r.alt != null
                ? out.split(r.raw).join(`![${r.alt}](${url})`)
                : out.split(r.raw).join(r.raw.replace(r.src, url));
            }
          }
      ...
      return el("div", { className: "fx-preview-body" },
        el("div", { className: "fx-md-scroll fx-scroll" },
          el("div", { className: "fx-md" }, el(MarkdownText, { text: html }))),
        truncated
          ? el("div", { className: "fx-trunc-note" },
              "内容过长，仅渲染前 256 KiB，完整内容请用外部程序打开")
          : null,
